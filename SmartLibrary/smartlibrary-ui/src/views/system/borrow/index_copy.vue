<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="borrowList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="借阅序号" align="center" prop="borId" />
      <el-table-column label="标准书号" align="center" prop="bkIsbn" />
      <el-table-column label="读者身份证号" align="center" prop="rdIdNumber" />
      <el-table-column label="读者姓名" align="center" prop="rdName" />
      <el-table-column label="读者手机号" align="center" prop="rdPhone" />
      <el-table-column label="借阅日期" align="center" prop="borrowTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.borrowTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="应还日期" align="center" prop="shouldBackTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.shouldBackTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="归还日期" align="center" prop="returnTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.returnTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态" align="center" prop="bkState">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_borrow_state" :value="scope.row.bkState"/>
        </template>
      </el-table-column>
      <el-table-column label="是否超期" align="center" prop="bkOvertime">
        <template slot-scope="scope">
...
export default {
  name: "borrow",
  dicts: ['sys_borrow_state','sys_overTime_state'],
  data() {
    return {
      // 遮罩层
      loading: true,
